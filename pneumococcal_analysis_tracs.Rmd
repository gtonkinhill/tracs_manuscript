...
# Bin and set to mid point of bin
min_distances_filt_single$binned_geo <- (cut(min_distances_filt_single$geo_dist, 
                                          breaks = seq(0, 2000, by = 200), labels=FALSE,
                                          include.lowest = TRUE)-0.5)*200


ggplot(min_distances_filt_single %>% 
         filter(geo_dist<2000) %>%
         filter(`filtered SNP distance`<150),
       aes(y=`expected K`), colour='#377eb8') +
  geom_boxplot(aes(x=binned_geo, group=binned_geo), alpha=0.05, size = 0.2, colour='#377eb8') +
  geom_smooth(aes(x=geo_dist), method='loess', se=TRUE, colour='#377eb8') +
...
ggplot(min_distances_filt_single %>% 
  ...
  geom_boxplot(aes(x=binned_geo, group=binned_geo), alpha=0.05, size = 0.2) +
