...
This small section was used to choose samples from which to simulate the metagenomic data in the 'simulations_metagenomic_mixture.RmD' Rmarkdown.
```{r, eval=FALSE}
temp <- distances_filt %>% filter(`filtered SNP distance` < 3000)
temp <- distances_filt %>% filter(!paste(sampleA, sampleB) %in% paste(temp$sampleA, temp$sampleB))

# (distances %>% filter(paste(sampleA, sampleB) %in% paste(temp$sampleA, temp$sampleB)[[1]]))$strain

sim_pair <- paste(temp$sampleA, temp$sampleB)[[1]]
...
Plot SNP estimate distributions

```{r}
ggplot(pdf |> filter(distance < 3000) |> filter(method!='strainGE'), aes(x=distance, fill=relationship)) +
...
ggsave("./figures/FMT_snp_distance_method_comparison.png", width = 10, height = 7)
ggsave("./figures/FMT_snp_distance_method_comparison.pdf", width = 10, height = 7, device = cairo_pdf)
```
...
table(pdf$method[pdf$distance<pdf$threshold], pdf$bifi[pdf$distance<pdf$threshold])
